...
//--- 
      ...
      double MA1Array [];
      int MA1Definition = iMA(_Symbol,_Period,MA1Period,0,MODE_EMA,PRICE_CLOSE);
      ArraySetAsSeries(MA1Array,true);
      CopyBuffer(MA1Definition,0,0,3,MA1Array);
      double MA1Value = NormalizeDouble(MA1Array[0],2);
      
      if(PriceInfo[0].close > MA1Value && PositionsTotal()==0)// && str1.hour < 17)
      {
         isLong = true;
         int AskD = Ask / 0.1;
      ...
      else if(PriceInfo[0].close < MA1Value && PositionsTotal()==0)// && str1.hour < 17)
      {
         isLong = false;
         int BidD = Bid / 0.1;
      ...
      {
         if((isLong == true && PriceInfo[0].close < MA1Value) || (isLong == false && PriceInfo[0].close > MA1Value))
         {
            CloseBuySellSymbol();
         }
      }
